...
//Variable use to store if function is paused.
let isPaused = false;
...
//JQuery function used to hide and show slide show on button click.
$(document).ready(function(){
	$("#hideButton").click(function(){
		setPause(1000);
		$("#slideshow-img").hide("slow");
	});
...
$(document).ready(function(){
	$("#makeSmallerButton").click(function(){
		...
		setPause(1000);
		//Make border larger during animation
		document.getElementById("slideshow-img").style.border = "5px solid black";
		
		$("#slideshow-img").animate({width: "80%"}, "linear", function() {
			//Reset border to previous style after animation.
			setTimeout(function() {
				document.getElementById("slideshow-img").style.border = "1px solid black";
			}, 1000);
		});
	});
	$("#makeLargerButton").click(function(){
		//Pauses the image change while animating slideshow.
		//This makes animation smother so that image size does not change in the middle of it.
		setPause(2000);
...
//Helper function to set isPaused and times before unpause.
function setPause(time) {
	isPaused = true;
	setTimeout(function() {
		isPaused = false;
	}, time);
